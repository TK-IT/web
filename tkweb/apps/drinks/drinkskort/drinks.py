...
def generatebarcard(drinks):
    for currentingredients in drinks:
        ...
        if served.lower() == "drinksglas":
            yield "\t\t" + r"\serveret I et %s med is" % served.lower()
        elif served.lower() == "fadølsglas":
            yield "\t\t" + r"\serveret I et %s med is" % served.lower()
        else:
...
def generatemixingcard(drinks, columns, use_alternatives):
    ...
    yield r"\toprule %s \\" % " & ".join("\\bfseries " + name for name, key in columns)
    yield r"\midrule"
    ...
    for drinknumber, currentingredients in enumerate(drinks):
        ...
        ingredients = ", ".join(
            " ".join(part for part in spirit.split("-"))
            for spirit in currentingredients["spirit"]
        )
        ...
        fields = dict(
            name=name, ingredients=ingredients, soda=soda, served=served, price=price
        )
...
def main():
    ...
    parser.add_argument("filename", help="Text file containing drinks to display")
    parser.add_argument(
        "-v", "--verbose", action="store_true", help="Print unrecognized lines"
    )
    parser.add_argument(
        "-s", "--sortbarcards", action="store_true", help="Sort drinks by price"
    )
    parser.add_argument(
        "-a",
        "--alternative",
