...
from tkweb.apps.eval.evalmacros import parseTimeoutMonth, EvalMacroPreprocessor
...
class WikiArticleTimeout(models.Model):
    article = models.OneToOneField(Article, related_name="wikiArticleTimeout")
    ...
    def timeout(self):
        if self.timeoutMonth:
            ...
            return datetime.date(year=year, month=self.timeoutMonth, day=1)
        return None
...
def wikiArticle_callback(sender, instance, **kwargs):
    ...
    if instance.current_revision is not None:
        ...
        try:
            match = EvalMacroPreprocessor.find_macro_invocations(lines, "timeout")[0]
        ...
        try:
            match = EvalMacroPreprocessor.find_macro_invocations(lines, "updated")[0]
            title = match.args[0]
