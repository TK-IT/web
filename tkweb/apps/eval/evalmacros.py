...
def parseTimeoutMonth(month):
    ...
    pattern = r"(?P<month>(%s))" % "|".join(m for ml in MONTHS for m in ml)
...
class EvalMacroExtension(markdown.Extension):
    def extendMarkdown(self, md, md_globals):
        ...
        md.preprocessors.add("tk-macros", EvalMacroPreprocessor(md), ">html_block")
...
class EvalMacroPreprocessor(markdown.preprocessors.Preprocessor):
    ...
    def _get_pattern(method_names):
        pattern = (
            ...
            r"\[(?P<macro>%s)"
            % "|".join(re.escape(method_name) for method_name in method_names)
            +
    ...
    MacroInvocation = collections.namedtuple("MacroInvocation", "macro args full")
    ...
    def find_macro_invocations(article_content, method_name):
        ...
        return [
            EvalMacroPreprocessor._parse_match(mo)
    ...
    def begin_hide(self, title="BEST", full=""):
        expanded = any(t.lower() == title.lower() for t in self.get_user_groups())
    ...
    begin_hide.meta = {
        ...
        "help_text": (
            "Skjuler en sektion der kun er relevant for nogle "
        ...
        "args": {"title": "Personen eller gruppen indholdet ikke er skjult for."},
    }
    ...
    fixme.meta = {
        ...
        "args": {"message": "Teksten til fixme noten"},
    }
    ...
    timeout.meta = {
        ...
        "args": {"month": "Måneden hvor artiklen forfalder."},
    }
    ...
    updated.meta = {
        ...
        "args": {
            "title": (
                "Navet på personen eller gruppen der har udført " + "ajourførelsen."
            ),
            "dato": (
                "Datoen, i formatet YYYY-MM-DD, hvor ajourførelsen har "
                + "fundet sted."
            ),
        },
    }
    ...
    TK.meta = {
        ...
        "help_text": (
            ...
            + "".join(
                [
                    "<tr><td>[%s]</td><td>%s</td></tr>" % (f.__name__, f())
                    for f in _TKBRANDFUNCS
                ]
            )
            + "</table>"
        ),
    ...
    tk_prefix.meta = {
        ...
        "help_text": (
            "Følgene makroer giver et titel-prefix eller -postfix "
            ...
            + "<tr><th>Makro</th><th>Output</th></tr>"
            + (
                "<tr><td>[tk_prefix 2010]VC</td><td>%sVC</td></tr>"
                % tkbrand.tk_prefix(("", 2010))
            )
            + (
                "<tr><td>[tk_prefix 2010 VC]</td><td>%s</td></tr>"
                % tkbrand.tk_prefix(("VC", 2010))
            )
            + (
                "<tr><td>[tk_kprefix 2016]BEST</td><td>%sBEST</td></tr>"
                % tkbrand.tk_kprefix(("", 2016))
            )
            + (
                "<tr><td>[tk_kprefix 2016 BEST]</td><td>%s</td></tr>"
                % tkbrand.tk_kprefix(("BEST", 2016))
            )
            + (
                "<tr><td>KA$$[tk_postfix 2017]</td><td>KA$$%s</td></tr>"
                % tkbrand.tk_postfix(("", 2017))
            )
            + (
                "<tr><td>[tk_postfix 2017 KASS]</td><td>%s</td></tr>"
                % tkbrand.tk_postfix(("KASS", 2017))
            )
            + (
                "<tr><td>[tk_prepostfix SEKR 2015]</td><td>%s</td></tr>"
                % tkbrand.tk_prepostfix(("SEKR", 2015))
            )
            + (
                '<tr><td>[tk_email FUHØ 2010]</td><td><a href="mailto:%(e)s@TAAGEKAMMERET.dk">%(e)s@TAAGEKAMMERET.dk</a></td></tr>'
