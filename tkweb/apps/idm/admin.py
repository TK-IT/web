...
from tkweb.apps.idm.models import Profile, Group, Title
        ...
        for choice in super(TitlePeriodFilter, self).choices(cl):
            try:
                ...
                choice["display"] = tk.prepostfix(
                    ("BEST/FU", period), gfyear=gfyear, prefixtype="unicode"
                )
            yield choice
...
class ProfileAdmin(admin.ModelAdmin):
    ...
    def get_titles(self, profile):
        ...
        if titles:
            with tk.set_gfyear(config.GFYEAR):
                return ", ".join(
                    sorted(tk.prepostfix(t, prefixtype="unicode") for t in titles)
                )
    ...
    def on_mailing_list(self, profile):
        return any(g.regexp == Group.REGEXP_MAILING_LIST for g in profile.groups.all())
...
class GroupAdmin(admin.ModelAdmin):
    ...
    list_display = ("name", "regexp", "matchtest", "members")
...
class TitleAdmin(admin.ModelAdmin):
    list_display = ("get_display_title", "profile_link", "get_period", "get_email")
    ...
    def profile_link(self, title):
        return format_html(
            '<a href="{}">{}</a>',
            reverse("admin:idm_profile_change", args=(title.profile_id,)),
            title.profile,
        )
