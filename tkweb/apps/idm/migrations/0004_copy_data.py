...
def copy_membership(apps, schema_editor):
    ...
    for o in Gruppemedlemmer.objects.values("gruppeid", "personid"):
        a.append(
            Profile.groups.through(profile_id=o["personid"], group_id=o["gruppeid"])
        )

    b = []
    qs = Gradgruppemedlemmer.objects.values("personid", "grad", "gruppeid")
    for o in qs:
        b.append(
            GradGroupMembership(
...
class Migration(migrations.Migration):
    ...
    dependencies = [("idm", "0003_create_relations")]

    operations = [
        migrations.RunPython(copy_membership, lambda apps, schema_editor: None),
        migrations.RunPython(copy_titles, lambda apps, schema_editor: None),
    ]
