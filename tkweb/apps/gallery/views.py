...
def gallery(request, **kwargs):
    ...
    allalbums_public = allalbums_allmedia.filter(basemedia__visibility=BaseMedia.PUBLIC)

    public_count = dict(
        allalbums_public.annotate(count=Count("basemedia")).values_list("id", "count")
    )
    ...
    context = {"years": years, "show_year": show_year, "albumSets": albumSets}
...
def upload(request):
    ...
    try:
        ...
        jfu_msg = {"name": file.name, "size": file.size, "error": error}
