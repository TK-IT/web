...
def coalesce_kinds(apps, schema_editor):
    ...
    for key, ids in kind_ids.items():
        ...
        purchase_qs = Purchase.objects.filter(kind_id__in=coalesce)
        purchase_qs.update(kind_id=main_id)
        coalesce_qs = PurchaseKind.objects.filter(id__in=coalesce)
        print("Coalesce", key, coalesce_qs.delete())


class Migration(migrations.Migration):

    dependencies = [("regnskab", "0010_purchasekind_meta")]

    operations = [migrations.RunPython(coalesce_kinds)]
