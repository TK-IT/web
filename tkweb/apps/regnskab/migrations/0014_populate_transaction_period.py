...
def populate_transaction_period(apps, schema_editor):
    ...
    date_to_period = dict(Sheet.objects.all().values_list("end_date", "period"))
    ...
    for id, time, session_period in qs:
        ...
        if time_period is None and session_period is None:
            raise Exception("No period for id=%s time=%r" % (id, time))
        if time_period is not None and session_period is not None:
            if time_period != session_period:
                raise Exception(
                    "Time (%r) disagrees with session (%r)"
                    % (time_period, session_period)
                )
        Transaction.objects.filter(id=id).update(period=time_period or session_period)


class Migration(migrations.Migration):

    dependencies = [("regnskab", "0013_transaction_period")]

    operations = [migrations.RunPython(populate_transaction_period)]
