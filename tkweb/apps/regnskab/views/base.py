...
class Home(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            email_template = EmailTemplate.objects.get(name="Standard")
...
def already_sent_view(request, regnskab_session):
    ...
    return TemplateResponse(request, "regnskab/already_sent.html", context=context)
...
class SessionCreate(TemplateView):
    ...
    def post(self, request):
        ...
        session.save()
        logger.info("%s: Opret ny opgørelse id=%s", self.request.user, session.pk)
...
class SheetCreate(FormView):
    ...
    @regnskab_permission_required_method
    def dispatch(self, request, *args, **kwargs):
        self.regnskab_session = get_object_or_404(Session.objects, pk=kwargs["session"])
    ...
    def get_initial(self):
        ...
        return dict(kinds="\n".join("%s %s" % x for x in kinds), period=config.GFYEAR)
    ...
    def form_valid(self, form):
        ...
        kinds = [
            PurchaseKind.get_or_create(
                name=kind["name"], position=i + 1, unit_price=kind["unit_price"]
            )
        ...
        logger.info(
            "%s: Opret ny krydsliste id=%s i opgørelse=%s " + "med priser %s",
            self.request.user,
            sheet.pk,
            self.regnskab_session.pk,
            " ".join("%s=%s" % (k["name"], k["unit_price"]) for k in data["kinds"]),
        )
...
class SheetRowUpdate(FormView):
    ...
    def get_initial(self):
        return dict(
            start_date=self.sheet.start_date,
    ...
    def clean(self, data_json):
        ...
        profile_ids = set(row["profile_id"] for row in row_data if row["profile_id"])
        profiles = {p.id: p for p in Profile.objects.filter(id__in=sorted(profile_ids))}
        missing = profile_ids - set(profiles.keys())
        if missing:
            raise ValidationError("Unknown profile IDs %s" % (missing,))
        return [
            dict(
                profile=row["profile_id"] and profiles[row["profile_id"]],
                name=row["name"],
                position=i + 1,
                image_start=row["image"] and row["image"]["start"],
                image_stop=row["image"] and row["image"]["stop"],
                kinds=[
                    Purchase(kind=kind, count=c or 0)
    ...
    def save_rows(self, rows):
        ...
        for row in delete:
            logger.info(
                "%s: Slet række %s i krydsliste %s: %r %r %s",
                self.request.user,
                row["position"],
                sheet.pk,
                row["name"],
                str(row["profile"]),
                " ".join("%s=%s" % (c.kind.name, c.count) for c in row["kinds"]),
            )
        ...
        for o in save:
            save_rows.append(
                SheetRow(
                    ...
                    name=o["name"],
                    position=o["position"],
                    image_start=o["image_start"],
                    image_stop=o["image_stop"],
                )
            )
            for c in o["kinds"]:
                if c.count:
            ...
            logger.info(
                "%s: Gem række %s i krydsliste %s: %r %r %s",
                self.request.user,
                o["position"],
                sheet.pk,
                o["name"],
                str(o["profile"]),
                " ".join("%s=%s" % (c.kind.name, c.count) for c in o["kinds"]),
            )
...
class Sortable:
    ...
    def __init__(self, sort_order_input, valid_keys):
        self.keys = []
        self.signs = []
        sort_order_list = sort_order_input.split(",") if sort_order_input else ()
    ...
    def key_func(self, value):
        return [s * (value.get(k) or 0) for k, s in zip(self.keys, self.signs)]
    ...
    def change_order_key(self, key):
        ...
        return ",".join(("-" if s == -1 else "") + k for k, s in zip(keys, signs))
...
class PurchaseStatsTable:
    ...
    def all_columns(self):
        return itertools.chain(
            self.columns_before, self.purchase_columns, self.columns_after
        )
        ...
        if self.sorter is None:
            ...
            for (key, places), label in zip(keys_places, labels):
                if places is None:
                    ...
                    header_cells.append(
                        format_html(fmt, qs=qs.urlencode(), c=key, h=label)
                    )
            header = format_html_join("\n", "{}", zip(header_cells))
...
class SessionList(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        by_year = sum_matrix(
            Purchase.objects.all(), "kind__name", "row__sheet__period", "count"
        )
        by_year.update(
            sum_matrix(Transaction.objects.all(), "kind", "period", "amount")
        )
        ...
        for r in period_table.add_data(by_year):
            r["period"] = format_html('<a href="?year={0}">{0}</a>', r["key"])

        purchases_by_session_qs = Purchase.objects.filter(
            row__sheet__session__period=period
        )
        by_session = sum_matrix(
            purchases_by_session_qs, "kind__name", "row__sheet__session_id", "count"
        )
        purchases_by_sheet_qs = Purchase.objects.filter(
            row__sheet__session=None, row__sheet__period=period
        )
        by_sheet = sum_matrix(
            purchases_by_sheet_qs, "kind__name", "row__sheet_id", "count"
        )

        transactions_by_session_qs = Transaction.objects.filter(session__period=period)
        by_session.update(
            sum_matrix(transactions_by_session_qs, "kind", "session_id", "amount")
        )
        transactions_by_sheet_qs = Transaction.objects.filter(
            session=None, period=period
        )
        by_sheet_time = sum_matrix(transactions_by_sheet_qs, "kind", "time", "amount")
        ...
        sheets = {s.pk: s for s in Sheet.objects.filter(session=None, period=period)}
        for row in session_table.add_data(by_sheet, sheets):
            sheet = row["key"]
            href = reverse("regnskab:sheet_detail", kwargs=dict(pk=sheet.id))
            row["raw_date"] = sheet.end_date.toordinal()
            row["date"] = format_html(
                '<a href="{}">Udsendt {}</a>', href, sheet.end_date
            )

        sessions = {s.pk: s for s in Session.objects.filter(period=period)}
        for row in session_table.add_data(by_session, sessions):
            session = row["key"]
            href = reverse("regnskab:session_update", kwargs=dict(pk=session.id))
            if session.sent:
                s = "Udsendt"
            ...
            row["date"] = format_html('<a href="{}">{} {}</a>', href, s, date)
...
class SessionUpdate(FormView):
    ...
    def get_initial(self):
        ...
        if email_template:
            ...
            return dict(
                subject="", format=EmailTemplate.POUND, markup=EmailTemplate.PLAIN
            )
    ...
    def get_sheets(self):
        purchases_by_sheet_qs = Purchase.objects.filter(row__sheet__session=self.object)
        by_sheet = sum_matrix(
            purchases_by_sheet_qs, "kind__name", "row__sheet_id", "count"
        )
    ...
    def form_valid(self, form):
        ...
        if save_it:
            ...
            self.object.save()
            logger.info(
                "%s: Ret emailskabelon %s for opgørelse %s",
                self.request.user,
                self.object.email_template_id,
                self.object.pk,
            )
        ...
        context_data = self.get_context_data(form=fresh_form, success=True)
...
class ProfileDetail(TemplateView):
    ...
    def post_action_remove_status(self):
        if not self.sheetstatus:
            return self.post_error("Personen allerede fjernet fra krydslisten")
        logger.info("%s: Fjern %s fra krydslisten", self.request.user, self.profile)
    ...
    def post_action_add_status(self):
        if self.sheetstatus:
            return self.post_error("Personen allerede tilføjet til krydslisten")
        logger.info("%s: Tilføj %s til krydslisten", self.request.user, self.profile)
    ...
    def post_action_add_alias(self):
        ...
        if s:
            logger.info(
                "%s: Tilføj alias %r til %s", self.request.user, s, self.profile
            )
    ...
    def post_action_set_primary_alias(self):
        ...
        if k == "real_title":
            return self.post_error(
                "Kan ikke ændre vist titel for " + "person med rigtig titel"
            )
        ...
        if s == current_alias_display:
            ...
            if current_alias:
                logger.info(
                    "%s: Fjern primær alias %r fra %s",
                    self.request.user,
                    current_alias_display,
                    self.profile,
                )
            ...
            if s:
                logger.info(
                    "%s: Tilføj primær alias %r til %s",
                    self.request.user,
                    s,
                    self.profile,
                )
                existing_same = Alias.objects.filter(
                    profile=self.profile, root=s, end_time=None
                )
    ...
    def post_default(self):
        for k in self.request.POST:
            if k.startswith(self.REMOVE_ALIAS):
                ...
                try:
                    o = Alias.objects.get(profile=self.profile, pk=pk)
    ...
    def get_sheets(self):
        ...
        qs = qs.annotate(amount=F("kind__unit_price") * F("count"))
        ...
        qs = qs.values("sheet", "date", "count", "kind__name", "amount", "session")
        ...
        for sheet_id, xs in groups:
            ...
            name = ", ".join(
                "%s× %s" % (floatformat(o["count"]), o["kind__name"]) for o in xs
            )
    ...
    def get_emails(self):
        ...
        for o in emails:
            href = reverse(
                "regnskab:email_detail",
                kwargs=dict(pk=o["session_id"], profile=self.profile.id),
            )
            amount = None
            date = o["time"].date()
            name = "Email"
    ...
    def get_names(self):
        ...
        for o in self.profile.alias_set.all():
            ...
            if o.end_time is None:
                ...
                names.append(
                    dict(name=name, since="Fra %s til %s" % (start, end), remove=None)
                )
        return names
    ...
    def get_alias_data(self):
        ...
        if title_qs:
            return ("real_title", tk.prefix(title_qs[0], config.GFYEAR, type="unicode"))
    ...
    def get_transaction_kind(self):
        try:
            ...
            raise ImproperlyConfigured(
                "TransactionBatchCreateBase subclass must define " + "transaction_kind."
            )
    ...
    def form_valid(self, form):
        ...
        for profile, amount, selected in form.profile_data():
            if selected:
                ...
                logger.info(
                    "%s: Sæt %r for %s i opgørelse %s til %s",
                    self.request.user,
                    o.get_kind_display(),
                    profile,
                    self.regnskab_session.pk,
...
class PurchaseNoteList(TemplateView):
    ...
    @regnskab_permission_required_method
    def dispatch(self, request, *args, **kwargs):
        self.regnskab_session = get_object_or_404(Session.objects, pk=kwargs["pk"])
        if not self.regnskab_session or self.regnskab_session.sent:
            return already_sent_view(request, self.regnskab_session)
        return super().dispatch(request, *args, **kwargs)
    ...
    def get_context_data(self, **kwargs):
        ...
        for note in sorted(note_amounts.keys()):
            ...
            amount = next(a for a, c in counter.items() if c == max_count)
...
class PurchaseBatchCreate(TransactionBatchCreateBase):
    ...
    def dispatch(self, request, *args, **kwargs):
        try:
            ...
            return redirect(
                "regnskab:purchase_note_list", pk=self.get_regnskab_session().pk
            )
        return super().dispatch(request, *args, **kwargs)
...
class PaymentPurchaseList(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        payment_qs = Transaction.objects.filter(
            session=self.regnskab_session, kind=Transaction.PAYMENT
        ).order_by()
        ...
        profile_sheets = {}
        purchase_qs = Purchase.objects.filter(row__sheet__session=self.regnskab_session)
        purchase_qs = purchase_qs.annotate(amount=F("kind__unit_price") * F("count"))
        purchase_qs = purchase_qs.annotate(profile_id=F("row__profile_id"))
        purchase_qs = purchase_qs.annotate(sheet_id=F("row__sheet_id"))
        ...
        initial_balances = compute_balance(
            profile_ids=profile_ids, created_before=self.regnskab_session.created_time
        )

        sheet_ids = set(
            s_id for p_id, sheets in profile_sheets.items() for s_id in sheets.keys()
        )
