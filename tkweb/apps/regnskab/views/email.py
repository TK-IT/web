...
class EmailTemplateUpdate(UpdateView):
    ...
    def form_valid(self, form):
        ...
        form.save()
        logger.info("%s: Ret emailskabelon %s", self.request.user, self.kwargs["pk"])
...
class EmailTemplateCreate(CreateView):
    ...
    def get_initial(self):
        ...
        try:
            ...
            return dict(subject="[TK] Status på ølregningen", name=name)
        else:
            return dict(
                subject=email_template.subject, name=name, body=email_template.body
            )

    def form_valid(self, form):
        o = form.save()
        logger.info("%s: Opret emailskabelon %s", self.request.user, o.pk)
        return redirect("regnskab:email_template_list")
...
class NewsletterCreate(FormView):
    ...
    def form_valid(self, form):
        ...
        newsletter = Newsletter(
            email_template=template, period=config.GFYEAR, created_by=self.request.user
        )
        newsletter.save()
        try:
            template.clean()
...
class NewsletterUpdate(FormView):
    ...
    def form_valid(self, form):
        ...
        return redirect("regnskab:newsletter_update", pk=self.object.pk)
...
class NewsletterEmailList(EmailListBase):
    ...
    @regnskab_permission_required_method
    def dispatch(self, request, *args, **kwargs):
        self.object = get_object_or_404(Newsletter.objects, pk=kwargs["pk"])
        return super().dispatch(request, *args, **kwargs)
    ...
    for email in emails:
        ...
        for i, image in enumerate(images):
            if image.shape[1] < image_width:
                images[i] = np.pad(
                    image, [(0, 0), (0, image_width - image.shape[1])], "maximum"
                )
...
class EmailDetail(DetailView):
    ...
    @regnskab_permission_required_method
    def dispatch(self, request, *args, **kwargs):
        self.regnskab_session = get_object_or_404(Session.objects, pk=self.kwargs["pk"])
        self.profile = get_object_or_404(Profile.objects, pk=self.kwargs["profile"])
        return super().dispatch(request, *args, **kwargs)
...
class EmailSend(View):
    ...
    def post(self, request, pk, profile=None):
        ...
        override_recipient = len(messages) == 1 and self.request.POST.get(
            "override_recipient"
        )
        ...
        if profile:
            logger.info(
                "%s: Send email for %s i opgørelse %s til %s",
                self.request.user,
                p,
                regnskab_session.pk,
                override_recipient,
            ...
            logger.info(
                "%s: Send emails i opgørelse %s", self.request.user, regnskab_session.pk
            )
...
class NewsletterEmailSend(View):
    ...
    def post(self, request, pk, profile=None):
        ...
        if profile is None:
            ...
            p = get_object_or_404(Profile.objects, pk=profile)
            qs = NewsletterEmail.objects.filter(newsletter=newsletter, profile=p)
        ...
        if profile:
            logger.info(
                "%s: Send email for %s i nyhedsbrev %s til %s",
                self.request.user,
                p,
                newsletter.pk,
                override_recipient,
            ...
            logger.info(
                "%s: Send emails i nyhedsbrev %s", self.request.user, newsletter.pk
            )
        email_backend.send_messages(messages)
