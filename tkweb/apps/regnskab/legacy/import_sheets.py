...
def get_profiles(data):
    ...
    return [
        Profile(name=name, email=email if "@" in email else "")
...
def get_existing_profiles(data, helper):
    ...
    existing_email = [
        (p.name, p.email, existing.get(p.name, Profile(email=None)).email)
        for p in profiles
    ]
    wrong_email = [x for x in existing_email if x[2] is not None and x[1] != x[2]]
    if wrong_email:
    ...
    new = sorted((name, email) for name, email, e in existing_email if e is None)
    if new:
...
purchase_kind_names = dict(oel="øl", xmas="guldøl", vand="sodavand", kasser="ølkasser")
...
def get_sheets(data, profiles):
    ...
    for o in data:
        time = strptime(o["time"])
        sheet = Sheet(start_date=time.date(), end_date=time.date(), period=o["period"])
        sheets.append(sheet)
        kind_map = {}
        for i, kind in enumerate(o["kinds"]):
            ...
            purchase_kinds.append(
                PurchaseKind(
                    sheet=sheet, position=i + 1, name=name, unit_price=kind["price"]
                )
            )
        ...
        for i, name in enumerate(sorted(o["purchases"])):
            ...
            for k, v in o["purchases"][name].items():
                purchases.append(Purchase(row=row, kind=kind_map[k], count=v))
...
def import_sheets(data, helper):
    ...
    sheets = save_all(sheets, unique_attrs=("start_date", "end_date"), only_new=True)
    raise NotImplementedError("PurchaseKind.sheet no longer exists")
    purchase_kinds = filter_related(sheets, purchase_kinds, "sheet")
    purchase_kinds = save_all(
        purchase_kinds, unique_attrs=("sheet", "name"), only_new=True
    )
    rows = filter_related(sheets, rows, "sheet")
    rows = save_all(rows, unique_attrs=("sheet", "name", "profile"), only_new=True)
...
if __name__ == "__main__":
    ...
    with open(os.path.join(BASE_DIR, "manage.py")) as fp:
        settings_line = next(l for l in fp if "DJANGO_SETTINGS_MODULE" in l)
