...
PersonBase = collections.namedtuple(
    "Person", "navn titel aliaser email senest total gaeld skjul sendEmail"
)

ForbrugBase = collections.namedtuple("Forbrug", "kasser xmas oel vand andet betalt")

PriserBase = collections.namedtuple("Priser", "oel xmas vand kasser")
...
def get_amount(priser, forbrug):
    return (
        sum(getattr(priser, k) * getattr(forbrug, k) for k in Priser._fields)
        + forbrug.andet
    )
...
def read_regnskab(fp):
    ...
    if o1 == b"<<<<":
        raise ValueError("Data starts with '<<<<', merge conflict?")
    if o1 != header1:
        if len(o1) != len(header1):
            raise ValueError("Incomplete read: %d != %d" % (len(o1), len(header1)))
        version = 0
