        ...
        try:
            ...
            if qs:
                if len(qs) == 1:
                    ...
                    print("%s: No exact match for %s; found %s" %
                          (o['root'], (o['start_time'], o['end_time']),
                           [(a.start_time, a.end_time) for a in qs]), file=fp)
            else:
                # print("%s: No such alias" % o['root'], file=fp)
                pass
        else:
            if a.profile.name != o['name']:
                print("%s %s: Expected %s, found %s" %
                      (o['root'], a.id, o['name'], a.profile.name), file=fp)
            else:
