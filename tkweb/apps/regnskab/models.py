...
def get_inka():
    try:
        return Profile.objects.get(title__root="INKA", title__period=config.GFYEAR)
...
class Transaction(models.Model):
    ...
    session = models.ForeignKey(
        "Session", on_delete=models.CASCADE, null=True, blank=False
    )
...
class Sheet(models.Model):
    ...
    name = models.CharField(
        max_length=200, blank=True, help_text="f.eks. HSTR, revy, matlabotanisk have"
    )
    ...
    image_file = models.FileField(upload_to=sheet_upload_to, blank=True, null=True)
    row_image_width = models.PositiveIntegerField(blank=True, null=True)
    row_image = models.FileField(upload_to=sheet_upload_to, blank=True, null=True)
    ...
    def rows(self):
        ...
        titles = get_primary_titles(profile_ids=profile_ids, period=self.period)
        ...
        for row in result:
            try:
                ...
                row["title_name"] = row["profile"].name if row["profile"] else ""
            else:
                row["display_title"] = (
...
class PurchaseKind(models.Model):
    ...
    name = models.CharField(max_length=200, help_text="f.eks. guldøl, guldølskasser")
    unit_price = models.DecimalField(
        max_digits=12, decimal_places=2, help_text="f.eks. 8, 10, 13, 200, 250"
    )
    ...
    def get_or_create(cls, name, position, unit_price):
        qs = cls.objects.filter(name=name, position=position, unit_price=unit_price)
        try:
            kind = qs[0]
        except IndexError:
            kind = PurchaseKind(name=name, position=position, unit_price=unit_price)
            kind.save()
        return kind
...
class SheetRow(models.Model):
    ...
    def image_data(self):
        ...
        if not row_image or not row_image_width:
            return None, None, None, None
        return (row_image.url, row_image_width, self.image_start, self.image_stop)
...
class Purchase(models.Model):
    ...
    count = models.DecimalField(
        max_digits=9, decimal_places=4, help_text="antal krydser eller brøkdel"
    )
...
def compute_balance(
    profile_ids=None, created_before=None, *, output_matrix=False, purchases_after=None
):
    purchase_qs = Purchase.objects.all()
    if created_before:
        purchase_qs = purchase_qs.filter(row__sheet__created_time__lt=created_before)
    if profile_ids:
        purchase_qs = purchase_qs.filter(row__profile_id__in=profile_ids)
    purchase_qs = purchase_qs.exclude(row__profile_id=None)
    balance = sum_vector(
        purchase_qs, "row__profile_id", F("count") * F("kind__unit_price")
    )
    ...
    if output_matrix:
        ...
        purchases = sum_matrix(purchase_qs, "kind__name", "row__profile_id", "count")
        purchases.update(sum_matrix(transaction_qs, "kind", "profile_id", "amount"))
...
def _process_inlines(body_html, cb):
    ...
    def repl(mo):
        ...
        try:
            inline = EmailTemplateInline.objects.get(pk=inline_pk, hash=hash)
...
def body_html_inlines(body_html):
    ...
    invis = "data:image/gif;base64," + "R0lGODlhAQABAAAAACH5BAEAAAAALAAAAAABAAEAAAI="
    inlines = []
        ...
        def cb(inline):
            if inline is not None:
                return "data:%s;base64,%s" % (
                    inline.mime_type,
                    base64.b64encode(inline.blob).decode(),
                )
...
def regenerate_emails(email_set):
    ...
    if email_set.sent:
        raise ValidationError("Tried to regenerate emails for session already sent")
    ...
    if email_set.email_template.markup == EmailTemplate.HTML:
        ...
        if "#SKJULNUL:" in html_template:
            ...
            raise ValidationError("#SKJULNUL:# kan ikke bruges i HTML-emails")
...
def get_base_email_context(email_set, profile_data):
    ...
    if primary_title:
        title = (
            tk.prefix(primary_title, email_set.period, type="unicode")
            if primary_title.period
            else primary_title.root
        )
    else:
        title = None
...
def regenerate_email(email_set, profile_data):
    ...
    try:
        ...
        if email_set.email_template.markup == EmailTemplate.HTML:
            email.body_html = format(email_set.email_template.body_html(), context)
    except KeyError as exn:
        raise ValidationError("Emailskabelon har en ukendt variabel %r" % exn.args[0])
    if existing_email:
        changed_keys = [
            k for k in email_fields if getattr(email, k) != getattr(existing_email, k)
        ]
        if changed_keys:
...
class Session(models.Model):
    ...
    def get_recipient_data(self):
        ...
        payment_sums = sum_vector(
            transactions.filter(kind=Transaction.PAYMENT), "profile_id", "amount"
        )
        ...
        self._kind_price = {n: set(k.unit_price for k in g) for n, g in kind_groups}
        if not self._kind_price:
            self._kind_price = {n: {p} for n, p in get_default_prices()}

        purchases = Purchase.objects.filter(row__sheet__session=self)
        purchases = purchases.exclude(row__profile=None)
        pmatrix = sum_matrix(purchases, "row__profile_id", "kind__name", F("count"))
    ...
    def get_email_context(self, profile_data):
        ...
        if "guldølkasse" in purchase_count:
            guld_ratio = next(iter(kind_price["guldølkasse"])) / next(
                iter(kind_price["ølkasse"])
            )
            kasse_count += guld_ratio * purchase_count["guldølkasse"]
        if "sodavandkasse" in purchase_count:
            vand_ratio = next(iter(kind_price["sodavandkasse"])) / next(
                iter(kind_price["ølkasse"])
            )
...
def email_body_html_data_uris(email):
    ...
    if email.body_html is None:
        return format_html(
            '<pre style="white-space: pre-wrap">{}</pre>', email.body_plain
        )
...
class Email(models.Model):
    session = models.ForeignKey(
        Session, on_delete=models.CASCADE, related_name="email_set"
    )
    profile = models.ForeignKey(
        Profile, on_delete=models.SET_NULL, null=True, blank=False, related_name="+"
    )
    subject = models.TextField(blank=False)
...
class NewsletterEmail(models.Model):
    newsletter = models.ForeignKey(
        Newsletter, on_delete=models.CASCADE, related_name="email_set"
    )
    profile = models.ForeignKey(
        Profile, on_delete=models.SET_NULL, null=True, blank=False, related_name="+"
    )
...
def get_profiles_title_status(period=None, time=None):
    ...
    statuses = {
        pk: max(s, key=lambda s: (s.end_time is None, s.end_time)) for pk, s in groups
    }
    ...
    for p in profiles:
        ...
        p.in_current = p.status and (
            p.status.end_time is None or (time is not None and p.status.end_time > time)
        )
