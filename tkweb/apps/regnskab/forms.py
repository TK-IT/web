...
class SheetCreateForm(forms.Form):
    start_date = forms.DateField(label="På-dato", help_text="Format DD.MM.YYYY")
    end_date = forms.DateField(label="Af-dato", help_text="Format DD.MM.YYYY")
    image_file = forms.FileField(label="Scannet PDF", required=False)
    ...
    kinds = forms.CharField(widget=forms.Textarea, label="Priser")
...
class EmailTemplateForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        try:
            ...
            cleaned_data["body"] = EmailTemplateForm.convert_body(body, in_, out)
        return cleaned_data
...
class AnonymousEmailTemplateForm(forms.Form):
    subject = forms.CharField(
        max_length=200, initial="[TK] ", widget=forms.TextInput(attrs={"size": 60})
    )
    body = forms.CharField(widget=forms.Textarea(attrs={"cols": 70, "rows": 20}))
    format = forms.ChoiceField(
        choices=EmailTemplate.FORMAT, initial=EmailTemplate.POUND
    )
    markup = forms.ChoiceField(
        choices=EmailTemplate.MARKUP, initial=EmailTemplate.PLAIN
    )
    initial_markup = forms.ChoiceField(
        choices=EmailTemplate.MARKUP, widget=forms.HiddenInput()
    )
        ...
        for profile, amount, selected in profiles:
            ...
            if profile.title:
                profile.display_name = "%s %s" % (
                    ...
                    if profile.title.period
                    else profile.title.root,
                    profile.name,
                )
            else:
            ...
            self.fields[p + "selected"] = forms.BooleanField(
                initial=selected,
                required=False,
                label="%s markeret" % profile.display_name,
            )
            amount_str = "%g" % amount
            try:
                int(amount_str)
            ...
            self.fields[p + "amount"] = forms.FloatField(
                initial=amount_str,
                label="%s beløb" % profile.display_name,
...
class SheetRowForm(forms.Form):
    start_date = forms.DateField(label="På-dato", help_text="Format DD.MM.YYYY")
    end_date = forms.DateField(label="Af-dato", help_text="Format DD.MM.YYYY")
    data = forms.CharField(widget=forms.HiddenInput(attrs=dict(id="tk_rows")))


@placeholder_from_help
class ProfileListForm(forms.Form):
    purchases_after = forms.DateField(
        label="Forbrug siden", required=False, help_text="Format DD.MM.YYYY"
    )
