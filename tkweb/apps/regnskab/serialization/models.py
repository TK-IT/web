...
class AliasData(base("Alias")):
    parent_field = "profile"
    fields = ("root", "period", "is_title", "start_time", "end_time", "created_time")


class SheetStatusData(base("SheetStatus")):
    parent_field = "profile"
    fields = ("start_time", "end_time", "created_time")


class ProfileData(base("Profile")):
    fields = ("id", "name", "email")
    children = {"titles": TitleData, "aliases": AliasData, "statuses": SheetStatusData}


class EmailData(base("Email")):
    parent_field = "session"
    fields = ("profile", "subject", "body", "recipient_name", "recipient_email")
...
class SheetRowData(base("SheetRow")):
    ...
    children = {"purchases": PurchaseData}


class SheetData(base("Sheet")):
    parent_field = "session"
    fields = ("name", "start_date", "end_date", "period", "created_time")
    children = {"rows": SheetRowData, "kinds": SheetKindRelationData}
...
class TransactionData(base("Transaction")):
    parent_field = "session"
    fields = ("kind", "profile", "time", "period", "amount", "note", "created_time")
    bulk = True
...
class LegacySheetData(base("Sheet")):
    fields = ("name", "start_date", "end_date", "period", "created_time")
    children = {"rows": LegacySheetRowData, "kinds": SheetKindRelationData}
