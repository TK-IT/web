...
def main():
    ...
    parser.add_argument(
        "--nuke-database-before-loading", action="store_true", dest="nuke"
    )
    ...
    if args.nuke:
        ...
        if hostname not in ALLOW_NUKE_HOSTS:
            raise Exception("You need to add %r to ALLOW_NUKE_HOSTS." % hostname)

        import tkweb.apps.regnskab.models

        models = (
            """Transaction Purchase PurchaseKind Sheet Session Title Alias
