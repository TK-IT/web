...
parser = argparse.ArgumentParser(
    description="Delete all old Albums and Images. Transverses the rootdir and makes new Albums. With arguments it will also add Images."
)
parser.add_argument(
    "-c",
    "--check",
...
parser.add_argument(
    "rootdir", help="The path to the 'Billeder' folder from the old webpage"
)
...
replDict = {"_": " ", "ae": "æ", "AE": "Æ", "oe": "ø", "OE": "Ø", "aa": "å", "AA": "Å"}
...
deleteFiles = [
    "18-1415/90-AArets_gang/Originale/20140925-142750a.jpg",
...
for yearFolder in os.listdir(rootdir):
    ...
    for eventFolder in os.listdir(os.path.join(rootdir, yearFolder)):
        ...
        for orgiOrFile in os.listdir(os.path.join(rootdir, yearFolder, eventFolder)):
            if orgiOrFile in ("thumbs"):
                ...
                l = os.listdir(
                    os.path.join(rootdir, yearFolder, eventFolder, orgiOrFile)
                )
        ...
        if not missingFromOriginale:
            ...
            if missingFromResized:
                missingFromResizedGlobal.extend(
                    [
                        os.path.join(rootdir, yearFolder, eventFolder, f)
                        for f in missingFromResized
                    ]
                )
            ...
            if args.check or args.save:
                ...
                for filepath in filelist:
                    ...
                    if ext in (".png", ".gif", ".jpg", ".jpeg"):
                        ...
                        tmpfilename = (
                            os.path.splitext(os.path.basename(filepath))[0] + ".mp4"
                        )
                        tmpfilepath = os.path.join("/tmp", tmpfilename)
                        ffargs = (
                            ["./ffmpeg-3.1.1-64bit-static/ffmpeg", "-y", "-i", filepath]
                            + "-c copy -movflags +faststart".split()
                            + [tmpfilepath]
                        )
                        subprocess.check_call(ffargs)
                        op2 = open(tmpfilepath, "rb")
                        file2 = File(op2)
                        instance = GenericFile(
                            originalFile=file, file=file2, album=album
                        )
                        ...
                        ffargs = (
                            ["./ffmpeg-3.1.1-64bit-static/ffmpeg", "-y", "-i", filepath]
                            + "-c:v libx264 -pix_fmt yuv420p -profile:v baseline -level 3.0 -movflags +faststart".split()
                            + [tmpfilepath]
                        )
                        subprocess.check_call(ffargs)
                        op2 = open(tmpfilepath, "rb")
                    ...
                    try:
                        ...
                        print(
                            "=== %s ==="
                            % (os.path.join(rootdir, yearFolder, eventFolder))
                        )
                        print("file", file)
